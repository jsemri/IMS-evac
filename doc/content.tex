...
\begin{enumerate}
    ...
    value of normalizing coefficient $C_{SS}$ to be 0.15. 
...
In D1 case all exits were functional and those sitting in D105 used two closest
ones. Those exits are not very far and therefore an extraction of forty seconds
seems reasonable. In D2 a side exit from inside lecture room was blocked, but
the one outside was free, so it took a little longer to evacuate because of the
narrow entrance to D105. In D3 the same happens on the other side, which again
makes evacuation even longer. In D4 only main entrance is preserved, but
evacuation time is roughly the same; the reason for this is that, due to the
narrow entrances into D105 lecture room, a large queue is created
(see Image TODO) which seems to be the critical point. To confirm this, D5 case
was designed. The effect here is even more substantial, which can be seen on
Image TODO.
...
In the next experiment suite, we explore worst-case scenarios (D5 and E5) in
more detail by varying number of occupants from 100 to 500 and then varying
number of fire source from 1 to 3. The results are reported in the table TODO

\subsection{Results analysis}
Before making conclusions about quality of building plannings, let us first
analyse obtained results to see if they make sense.
