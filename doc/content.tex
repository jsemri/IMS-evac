...
\begin{enumerate}
    ...
    cells reachable by smoke (i.e. any cell but the wall) and $S$ is a number
    of adjacent cells affected by smoke and $C_{SS}$ is smoke spreading coefficient.
    ...
    reachable by smoke that is on the boundary of the automaton, i.e.
    probability will be always proportional to $\frac{S}{T}$.
...
FIT VUT campus.
In both cases we are interested if building planning is suitable for carrying
out evergency evacuations.

For each of the wings we created its bitmap analog using the plans
...
In each experiment we compute a total evacuation time (time interval between
alarm going off and extraction of the last individual).
...
There are two narrow entrances to the lecture room from the entrance hall.
There are a total of five exits from the building, a primary one on the east
side, a pair of exits on the north/south side of the vestibule and a similar
pair inside the lecture room.
Note that a south vestibul exit leads to the corridor between D and A segments
where an actual exit is placed.
...
There are two narrow entrances to the E112, and one entrance into E104 and
E105 each.
...
attendance + someone in the halls) and one random source of fire.
...
Result for each case reporting total evacuation time and a maximum smoke
exposure time are reported in Table TODO

Before judging building plans, let us first inspect the result to see if they
make sense.
...
reasonable.
In D2 one of side exits from inside lecture room was blocked, but the one
outside was free, so it took quite longer to evacuate because of the
narrow entrance to D105 in the way.
...
the reason for this is that, due to the narrow entrances into D105 lecture room,
a large queues have been creating there, see Image TODO.
In D5 one of this entrances is blocked and the effect here is even more
apparent, see Image TODO.

Lecture room entrances seem to be a critical place;  to confirm this, let's
increase the width of the entrance and see what would be the impact.
We will use modifications of D5 case and vary door width from 3 to 5.
Results are displayed in the table TODO; results for D5 (door width of 2) are
repeated there for comparison.

It is clear that enlarging entrance(s) width will result in significantly more
successful evacuations.
Nonetheless, even for worst cases, smoke exposure rates are way below the
threshold, which indicates that even in the worst case no one will suffer
substantial damage.

Let us now consider E wing. Here are the scenarios:
\begin{itemize}
...
more detail by varying number of occupants from 100 to 500 and then varying
number of fire sources from 1 to 3. The results are reported in the table TODO
